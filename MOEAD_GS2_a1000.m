classdef MOEAD_GS2_a1000 < ALGORITHM
...
%--------------------------------------------------------------------------
    ...
    methods
        function main(Algorithm,Problem)
            ...
            Population = Problem.Initialization(inipop);
            ...
            for i =1:Problem.N                
               switch type
                   case 1 
                     ...
                     normP   = sqrt(sum((temp_pop.objs-repmat(Z,inipop,1)).^2,2));
                     CosineP = sum((temp_pop.objs-repmat(Z,inipop,1)).*W(i,:),2)./normW./normP;
                     g = normP.*CosineP + 5*normP.*sqrt(1-CosineP.^2);
                     ind=find(g==min(g)); 
                     pop=[pop temp_pop(ind)];
                   case 2 
                       % Tchebycheff approach
                      g = max(abs(temp_pop.objs-repmat(Z,inipop,1)).*W(i,:),[],2);
                      ind=find(g==min(g)); 
                      pop=[pop temp_pop(ind)];
               end
            end 
