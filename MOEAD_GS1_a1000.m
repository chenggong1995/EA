classdef MOEAD_GS1_a1000 < ALGORITHM
            ...
            for i =1:Problem.N
               switch type
                   case 1 
                     ...
                     normP   = sqrt(sum((temp_pop.objs-repmat(Z,count,1)).^2,2));
                     CosineP = sum((temp_pop.objs-repmat(Z,count,1)).*W(i,:),2)./normW./normP;
                   ...
                   case 2 
                      ...
                      g = max(abs(temp_pop.objs-repmat(Z,count,1)).*W(i,:),[],2);
                      ind=find(g==min(g)); 
                      pop=[pop temp_pop(ind)];
                      temp_pop(ind)=[];
                      count=count-1;   
